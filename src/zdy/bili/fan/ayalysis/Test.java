...
public class Test {
	...
	private static void getInfo(int mid) throws IOException {
		BiliApiManager.getInstance().getUserInfo(mid, new Callback() {

			@Override
			public void onResponse(Call arg0, Response respone) throws IOException {
				// TODO Auto-generated method stub
				JsonArray currentArray;
				System.out.println("getInfo  code= " + respone.code() + "  status= " + respone.message());
				String raw = respone.body().string();
	...
	private static void getVideo(int mid) {
		...
		BiliApiManager.getInstance().getVideoList(mid, new Callback() {
			...
			public void onResponse(Call arg0, Response respone) throws IOException {
				// TODO Auto-generated method stub
				System.out.println("getVideo  code= " + respone.code() + "  status= " + respone.message());
				String raw = respone.body().string();
				JsonObject object = new JsonParser().parse(raw).getAsJsonObject().get("data").getAsJsonObject();
	...
	private static int[] getInfoNotRecursion(int mid) throws IOException {
		Response response = BiliApiManager.getInstance().getUserInfo(mid);
